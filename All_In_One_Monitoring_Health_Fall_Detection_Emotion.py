...
async def receive_sensor_data(queue, model, scaler):
    while True:
        try:
            ...
            print("Reconnecting to ESP32 in 5s...")
...
async def unified_monitor():
    ...
    header = (
        ...
        f"{'FALL STATUS':<15} | {'FALL CONF(%)':<13} | {'EMOTION':<12} | {'EMO CONF(%)':<11} | {'FACE (x,y,w,h)':<20} |"
    )
    print("=" * len(header))
    print(header)
    print("=" * len(header))
    ...
    while True:
        ...
        face_coords = shared_state['face']

        print(
            f"| {timestamp:<23} | {data['hr']:<5} | {data['spo2']:<6} | {data['temp']:<9.2f} | "
            f"{data['stage']:<14} | {fall_status:<15} | {fall_conf:<13.2f} | {emotion:<12} | {emo_conf:<11.2f} | {face_coords:<20} |"
        )
