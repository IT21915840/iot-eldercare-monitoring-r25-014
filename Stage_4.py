...
def load_sleep_model() -> Tuple[Any, Any]:
    try:
        ...
        logging.info("Sleep stage model/scaler loaded successfully.")
        return model, scaler
    except Exception as e:
        logging.error(f"Error loading sleep model/scaler: {e}")
        return None, None
...
def predict_emotion(face_roi: np.ndarray, model: Optional[tf.keras.Model]) -> Tuple[str, float]:
    try:
        if model is None:
            return "Unknown", 0.0
        if face_roi.size == 0 or face_roi.ndim < 3:
            return "No face ROI", 0.0
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        logging.info("Real-time data client disconnected")
...
async def unified_monitor() -> None:
    ...
    logging.info("--- Monitoring System Ready. Broadcasting data on ws://0.D.0.0:8000/ws ---")
    ...
    while True:
        ...
        fused_payload = {
            **data,
            ...
            "emotion_confidence": patient_state.get("emotion", ("---", 0.0))[1],
            "fall_confidence": patient_state.get("fall", ("---", 0.0))[1],
            "face_coords": patient_state.get("face", "---")
        }
...
if __name__ == "__main__":
    print("Starting Patient Monitoring Server (Data Caster)")
    print("Access real-time JSON data at WebSocket: ws://0.0.0.0:8001/ws")
