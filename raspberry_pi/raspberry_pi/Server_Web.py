...
MONGODB_URL      = "mongodb://192.168.0.102:27017/"
...
# Mount captures directory for static serving
...
async def list_captures(user_token: str = Depends(get_current_user)):
    """List all images in the captures directory."""
    try:
        files = [f for f in os.listdir(CAPTURES_DIR) if f.endswith(('.jpg', '.jpeg', '.png'))]
...
async def delete_capture(filename: str, user_token: str = Depends(get_current_user)):
...
@app.post("/api/trigger_fall")
async def trigger_fall_proxy(user_token: str = Depends(get_current_user)):
    """Proxy the manual fall trigger to Pi AI Server."""
    try:
        async with httpx.AsyncClient() as client:
            await client.post("http://localhost:8001/trigger_fall")
